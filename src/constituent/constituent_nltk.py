...
class constituent_decoder:
    ...
    def decode(self, labels, pos_tags, post_process, conflict_strategy):
        decoded_tree = self.decoder.decode(labels, pos_tags, fill_pos_nodes)
        if post_process:
            return self.postprocess_tree(decoded_tree, conflict_strategy)
        else:
            self.decoder.decode(labels, pos_tags, fill_pos_nodes)
    ...
    def decode(self, labels, pos_tags, preprocess_label, fill_pos_nodes):
        ...
        for label,pos_tag in zip(labels,pos_tags):
            ...
            if label.encoding_type==C_ABSOLUTE_ENCODING:
                ...
                # descend levels same way as c_absolute_decoder
            ...
            # If label was encoded in relative encoding
                ...
                # descend levels same way as c_relative_decoder
                if n_commons>0:
                    for level_index in range(n_commons): 
                        current_level.append(ParentedTree("NULL",[]))
                        current_level = current_level[-1]
