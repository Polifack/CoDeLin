...
class Adapter(transformers.PreTrainedModel):
    ...
    def __init__(self, config, classifiers=None, Z=None, labels_list=[]):
        ...
        print("[*] Initializing adapter with {} classifiers".format(len(self.classifiers)))
...
class Model(transformers.PreTrainedModel):
    def __init__(self, tasks, args):
        ...
        for i, task in enumerate(tasks):
            print("[*] Found task",i,"=>",task.name, "with", task.num_labels, "labels")
...
class Trainer(transformers.Trainer):
    def __init__(self, model, tasks, hparams, tokenizer=None, *args, **kwargs):
        ...
        # print("[*] Training with the following args:")
        # for k, v in trainer_args.__dict__.items():
        #     print(f"    - {k}: {v}")
    ...
    def predict(self, test_dataset = None, ignore_keys = None, metric_key_prefix = "test"):
        ...
        for i, task in enumerate(self.tasks):
            print("[*] Predicting task", i, "=>", task.name)
            
            self.compute_metrics = task.compute_metrics
            test_dataset = dict([fc.nth(i, self.test_dataset.items())])
            logits = super().predict(test_dataset, ignore_keys, metric_key_prefix)
            preds = logits.label_ids
            predictions[task.name] = preds
    ...
    def pipeline(self, task_index=0):
        print("[*] Calling pipeline with task_idx", task_index)
    ...
    def save_model(self, output_dir, task_index = 0, **kwargs):
        print("[*] Calling save_model with task_idx", task_index)
    ...
    def push_to_hub(self, repo, task_index=0, push_adapter=True):
        print("[*] Calling push_to_hub with task_idx", task_index)
        model, adapter = self.model.factorize(task_index = task_index)
