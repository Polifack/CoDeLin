...
def _get_action_list(tree):
    ...
    The tree must have NO unary chains (i.e. it must
    ...
    if siblings != []:
        ...
        if last_subtree is None:
            return last_word.label, last_postag.label, \
                Action(name="attach", target_node=0, parent_label=None, new_label=None), None
    ...
    if last_subtree.parent is None:
        return last_word.label, last_postag.label, \
            Action(name="attach", target_node=0, parent_label=parent_label, new_label=None), None 
...
class C_AttachJuxtapose(ACEncoding):
    def __init__(self, separator, unary_joiner, binary, binary_direction=None, binary_marker=None):        
    ...
    def __str__(self):
        return "Attach-Juxtapose Encoding"+\
            (f" binary{self.binary_direction}" if self.binary else "")

    def encode(self, tree: C_Tree):        
        tree = tree.collapse_unary(self.unary_joiner, intermediate_only=True)
        if self.binary:
            if self.binary_direction == "R":
                tree = C_Tree.to_binary_right(tree, self.binary_marker)
            elif self.binary_direction == "L":
                tree = C_Tree.to_binary_left(tree, self.binary_marker)
            else:
                raise Exception("Binary direction not supported")
        
        # print(f"default tree\t\t {tree}")
        lc_tree = LinearizedTree.empty_tree()
        seq = oracle_action_sequence(tree)
